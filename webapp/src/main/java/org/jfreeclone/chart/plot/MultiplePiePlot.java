package org.jfreeclone.chart.plot;
...
public class MultiplePiePlot extends Plot implements Cloneable, Serializable {
    ...
    /**
     * The number of columns to be used to layout the pies. Zero (0) value is
     * the default, which means that it will be automatically calculated
     * depending on the chart size and the number of pies to produce.
     */
    private int columns = 0;
    /**
     * The number of rows to be used to layout the pies. Zero (0) value is the
     * default, which means that it will be automatically calculated depending
     * on the chart size and the number of pies to produce.
     */
    private int rows = 0;
     ...
     * 
     * @param pieChart
     *            the pie chart (<code>null</code> not permitted).
     * 
     * @see #getPieChart()
     */
    ...
    public void draw(Graphics2D g2, Rectangle2D area, Point2D anchor,
        ...
        int displayCols = columns;
        int displayRows = rows;
        if (displayCols > 0) {
            if (displayCols > pieCount) {
                displayCols = pieCount;
            }
            displayRows = (int) Math.ceil((double) pieCount
                    / (double) displayCols);
        }
        else if (displayRows > 0) {
            if (displayRows > pieCount) {
                displayRows = pieCount;
            }
            displayCols = (int) Math.ceil((double) pieCount
                    / (double) displayRows);
        } else {
            // define the columns variable as always >= rows
            displayCols = (int) Math.ceil(Math.sqrt(pieCount));
            displayRows = (int) Math.ceil((double) pieCount
                    / (double) displayCols);
        ...
        for (int pieIndex = 0; pieIndex < pieCount; pieIndex++) {
            ...
            if (this.limit > 0.0) {
                piedataset = DatasetUtilities.createConsolidatedPieDataset(dd,
                        this.aggregatedItemsKey, this.limit);
            } else {
                piedataset = dd;
            }
    ...
    public LegendItemCollection getLegendItems() {
        ...
        while (iterator.hasNext()) {
            ...
            LegendItem item = new LegendItem(label, description, null, null,
                    getLegendItemShape(), paint, Plot.DEFAULT_OUTLINE_STROKE,
                    paint);
    ...
    /**
     * Gets the number of columns to be used to layout the pies. Zero (0) value
     * is the default, which means that it will be automatically calculated
     * depending on the chart size and the number of pies to produce.
     */
    ...
    /**
     * Sets the number of columns to be used to layout the pies. Zero (0) value
     * is the default, which means that it will be automatically calculated
     * depending on the chart size and the number of pies to produce.
     * 
     * Setting the columns has the effect to change the numbers of rows to auto,
     * as it depends on the number of pies to be drawn.
     */
     ...
     * depending on the chart size and the number of pies to produce.
     * 
     * Setting the rows has the effect to change the numbers of columns to auto,
     * as it depends on the number of pies to be drawn.
     */
