...
public class StyleGenerationParams {
    ...
    public org.geotools.styling.Style getGtStyle(SimpleFeatureCollection results) throws IOException {
        return getGtStyle(results, null);
    }
    
    public org.geotools.styling.Style getGtStyle(SimpleFeatureCollection results, Integer scale) throws IOException {
        ...
        if (getOverlayType() != null) {
            ...
            if (! getOverlayType().equalsIgnoreCase("symbol")) {
                ...
                FeatureTypeStyle overlays = getOverlays(results, scale);                
                if (overlays != null) {                    
                ...
                try {
                    propSymbols = getProportionalSymbols(results, scale);
                } catch (Exception e) {
    ...
    public FeatureTypeStyle getOverlays(SimpleFeatureCollection results, Integer scale) throws IOException {
        ...
        int size = getIconSize(overlayType, scale);
    ...
    /**
     * Try to produce a reasonable size depending on the scale and the overlay type.
     * Note that the size may be anyway inappropriate depending on the actual size
     * and proximity of the geometries being symbolized.
     * 
     * @param overlayType
     * @param scale
     * @return
     */
    private int getIconSize(String overlayType, Integer scale) {
        if (scale==null) {
            return 55;
        }
        if (overlayType.equals("pie")) {
            if (scale>40000000) {
                return 30;
            }
        ...
        else if (overlayType.equals("bar")) {
            if (scale>40000000) {
                return 40;
            }
            else if (scale>20000000) {
    ...
    public FeatureTypeStyle getProportionalSymbols(SimpleFeatureCollection results, Integer scale) throws Exception {
